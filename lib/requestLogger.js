...
module.exports = (options = {}) => async function requestLogger(ctx, next) {
    ...
    if (ctx.type === 'application/json')
        log.info(`leave: ${ctx.ip} requestId_ ${requestId}-${ctx.method} ${ctx.path} complete ${chalk.bold.white.bgMagenta(ctx.status)} at ${chalk.bold.cyan(moment() - start)}ms with body ${body}`);
    else
        log.info(`leave res: ${ctx.ip} requestId_${requestId}-${ctx.method} ${ctx.path} complete ${chalk.bold.white.bgMagenta(ctx.status)} at ${chalk.bold.cyan(moment() - start)}ms`);
};
